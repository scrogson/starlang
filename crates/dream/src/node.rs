...
//! managing distributed nodes in a DREAM cluster.
//!
//! # Overview
//!
...
//!
//! ```ignore
//! use dream::node::{self, ListOption};
//!
...
use dream_core::node::{is_distributed, node_name, NodeName};
...
pub fn current_atom() -> Atom {
    dream_core::node::node_name_atom()
}
...
///
/// ```ignore
/// use dream::node::{self, PingResult};
///
...
pub fn info(node: Atom) -> Option<dream_core::NodeInfo> {
...
///
/// **Note**: This is not yet implemented.
pub async fn spawn_monitor(_node: Atom, _module: &str, _function: &str, _args: Vec<u8>) -> Result<(Pid, dream_core::Ref), DistError> {
